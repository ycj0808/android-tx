...
public class XMPPHelper {
	...
	public static void verifyJabberID(String jid)
			throws ChatAddressMalformedException {
		if (jid != null) {
			Pattern p = Pattern
			...
			if (!m.matches()) {
				throw new ChatAddressMalformedException(
						"Configured Jabber-ID is incorrect!");
			}
		} else {
			throw new ChatAddressMalformedException("Jabber-ID wasn't set!");
		}
	}
	...
	public static CharSequence convertNormalStringToSpannableString(
		...
		while (localMatcher.find()) {
			...
			if (m - k < 8) {
				if (ChatApp.getInstance().getFaceMap().containsKey(str2)) {
					int face = ChatApp.getInstance().getFaceMap().get(str2);
